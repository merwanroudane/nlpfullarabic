# -*- coding: utf-8 -*-
"""
بناء رزنامة اجتماعات اللجنة الفدرالية للسوق المفتوحة (FOMC) بسحبها من الموقع الرسمي.

Builds the FOMC meeting calendar by scraping the official website.
...
def dump_df(df, filename="output"):
        '''
        تحفظ إطار البيانات الداخلي df في ملف pickle وملف csv.

        Dump the internal DataFrame df to a pickle file and a csv file.
        '''
...
def is_integer(n):
    '''
    تتحقّق مما إذا كان النصّ المُدخل قابلًا للتحويل إلى عدد صحيح.

    Check if an input string can be converted to an integer.
    '''
    try:
        float(n)
    except ValueError:
...
if __name__ == '__main__':
    ...
    # معالجة الوسيط الأول from_year — Handles the first argument, from_year
    if from_year:
    ...
    # جلب تواريخ اجتماعات اللجنة من الصفحة الحالية — Retrieve FOMC Meeting dates from the current page
    ...
    # جلب تواريخ اجتماعات اللجنة الأقدم من 2015 — Retrieve FOMC Meeting dates older than 2015
